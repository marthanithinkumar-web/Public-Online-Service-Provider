...
# Public service catalog. Keywords deliberately include common abbreviations,
# alternate spellings and search phrases so every seeded service is discoverable
# from the public search bar.
SERVICE_CATALOG = {
    'Identity & Citizen Documents': [
        ...
        ('Voter ID - Correction / Update', 'Assistance with eligible voter details corrections.', 'voter, voter id, epic, correction, update, name, dob, address'),
    ...
    'Entrance & Competitive Exams': [
        ...
        ('ECET Application Assistance', 'Assistance with engineering common entrance application for eligible candidates.', 'ecet, engineering common entrance, lateral entry'),
        ('ICET Application Assistance', 'Assistance with MBA/MCA entrance application.', 'icet, mba, mca, entrance exam'),
        ('PGECET Application Assistance', 'Assistance with postgraduate engineering entrance application.', 'pgecet, pg ecet, mtech, engineering entrance'),
        ('EdCET Application Assistance', 'Assistance with education entrance application.', 'edcet, ed cet, bed, education entrance'),
        ('LAWCET Application Assistance', 'Assistance with law entrance application.', 'lawcet, law cet, llb, llm, law entrance'),
        ('CPGET Application Assistance', 'Assistance with postgraduate entrance application.', 'cpget, pg entrance, osmania, postgraduate'),
...
with app.app_context():
    # Idempotent catalog seeding: existing deployments receive new services on
    # the next seed run without duplicating records.
    for category_name, services in SERVICE_CATALOG.items():
        ...
        for name, description, keywords in services:
            ...
            if not service:
                ...
                # Keep the catalog searchable and active while preserving any
                # admin-customized assistance price.
