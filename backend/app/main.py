...
def ensure_default_services():
    ...
    recharge=Service.query.filter_by(name='Recharge & Bill Payments').first()
    if recharge:
        recharge.official_fee_status='none';recharge.official_fee_inr=0.0
        if not recharge.keywords:recharge.keywords='recharge,mobile recharge,bill payment,bills,dth,fastag,electricity,water,lpg,gas,broadband,landline,postpaid,airtel,jio,vi,bsnl'
    db.session.commit()
