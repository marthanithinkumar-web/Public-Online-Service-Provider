...
def _purge_client_documents_when_order_closes(session, flush_context, instances):
    """Purge client-provided files when an application becomes terminal.

    Admin-delivered result files are preserved. Storage deletion runs before the
    terminal status transaction commits, so a storage failure blocks closure
    instead of silently retaining sensitive client uploads.
    """
