...
def _payable_breakdown(order):
    ...
    if status == 'unconfirmed':
        return None, {'assistance_fee_inr': assistance, 'official_fee_inr': None, 'official_fee_status': status}
    official = round(float(order.official_fee_inr or 0), 2) if status in {'known', 'none'} else 0.0
...
def payment_config():
    ...
    return jsonify({'provider': 'razorpay', 'enabled': bool(credentials), 'key_id': credentials[0] if credentials else None, 'currency': 'INR', 'purpose': 'request_total'})
...
def create_checkout(order_id):
    ...
    if total is None:
        return jsonify({'error': 'The official fee is not confirmed yet. Please wait for the administrator to confirm it so you can pay once.', 'breakdown': breakdown}), 409
    amount_paise = int(round(total * 100))
    if amount_paise <= 0:
        return jsonify({'message': 'Nothing is payable for this request.', 'payment': None, 'breakdown': breakdown}), 200
