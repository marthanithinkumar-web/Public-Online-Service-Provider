...
def _ensure_recharge_service(client):
...
def test_mobile_recharge_catalog_is_assistance_only(client):
    service_id, _ = _ensure_recharge_service(client)
...
def test_mobile_recharge_guided_fields_are_returned(client):
    service_id, _ = _ensure_recharge_service(client)
    response = client.get(f'/api/services/{service_id}')
    assert response.status_code == 200
    ...
    assert not any(fields[key].get('required') for key in ('mobile_number', 'operator', 'circle', 'plan_reference', 'recharge_amount'))
    assert requirements['documents'] == []


def test_mobile_recharge_is_searchable_by_operator(client):
    _ensure_recharge_service(client)
...
def test_recharge_fee_endpoint_has_no_official_fee(client):
    _ensure_recharge_service(client)
