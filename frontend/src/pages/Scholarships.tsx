...
import {fetchScholarships,Scholarship,scholarshipPath,scholarshipSourceLabel} from '../services/scholarships'

export default function Scholarships(){
  const [items,setItems]=useState<Scholarship[]>([]),[q,setQ]=useState(''),[loading,setLoading]=useState(true),[error,setError]=useState(''),[unavailableSources,setUnavailableSources]=useState<string[]>([])
  useEffect(()=>{let active=true;fetchScholarships().then(data=>{if(active){setItems(data.items);setUnavailableSources(Object.entries(data.discovery?.source_health||{}).filter(([,source])=>!source.ok).map(([key,source])=>source.source_name||key))}}).catch(()=>active&&setError('Scholarships are temporarily unavailable.')).finally(()=>active&&setLoading(false));return()=>{active=false}},[])
  const visible=useMemo(()=>{const tokens=q.toLowerCase().split(/\s+/).filter(Boolean);return items.filter(s=>tokens.every(t=>[s.title,s.provider,s.source_name,s.source_type,s.region,s.education_level,s.category,s.eligibility,s.academic_year].join(' ').toLowerCase().includes(t)))},[items,q])
  return <section className="content-section"><Seo title="Scholarships & Application Assistance" description="Search active scholarships, review eligibility and get application assistance." path="/scholarships" index/>
    ...
    {!loading&&!error&&unavailableSources.length>0&&<p className="info" role="status">Some official sources could not be checked: {unavailableSources.join(', ')}. Listings may be incomplete. Check the relevant official portal before applying.</p>}
