...
export default function ServiceDetail(){
 ...
 const [form,setForm]=useState({client_name:'',phone:'',email:'',notes:''});const [answers,setAnswers]=useState<Record<string,string>>({});const [file,setFile]=useState<File|null>(null);const [message,setMessage]=useState('');const [error,setError]=useState('');const [busy,setBusy]=useState(false);const [uploadMsg,setUploadMsg]=useState('');const [lastOrder,setLastOrder]=useState<any>(null);const [feeAccepted,setFeeAccepted]=useState(false);const [requestMode,setRequestMode]=useState<'guided'|'express'>('guided')
 ...
 const submit=async()=>{setError('');setMessage('');setBusy(true);try{const application_data={...(requestMode==='guided'?answers:{}),request_notes:requestMode==='guided'?form.notes.trim():'',service_name:service.name,request_mode:requestMode};const r=await axios.post(`${apiBase}/orders/`,{service_id:Number(service.id),application_data},{headers:{Authorization:`Bearer ${getToken()}`},timeout:SERVICE_TIMEOUT_MS});setLastOrder(r.data.order);setMessage(r.data.message);setStep(3)}catch(err:any){setError(err?.code==='ECONNABORTED'?'Submission timed out. Please try again.':apiErrorMessage(err?.response?.data?.error,'Unable to submit your request. Please try again.'))}finally{setBusy(false)}}
 ...
 const seo=<Seo title={`${service.name} Assistance`} description={seoDescription} path={canonicalPath} type="article" schema={serviceSchema}/>
 ...
 <div className="service-detail-hero"><span className="eyebrow">{service.category||'Public service'}</span><h1>{service.name}</h1><p>{service.description}</p><FeeSummary data={service}/></div>
  ...
  {step===1&&<section className="dashboard-section request-form simplified-request-form">
    ...
    <label>Notes<textarea rows={4} value={form.notes} onChange={event=>setForm({...form,notes:event.target.value})} placeholder="Anything else we should know?"/></label>
    <p className="request-safety-line">Do not enter OTPs, passwords, PINs or banking details. Documents can be added after submission.</p>
  ...
  {step===2&&<section className="dashboard-section simplified-review"><span className="eyebrow">Step 2</span><h2>Review and submit</h2><div className="request-summary"><p><strong>Service:</strong> {service.name}</p><p><strong>Client:</strong> {form.client_name} · {form.phone}</p>{requestMode==='guided'&&fields.filter(f=>answers[f.key]).map(f=><p key={f.key}><strong>{f.label}:</strong> {answers[f.key]}</p>)}{form.notes&&<p><strong>Notes:</strong> {form.notes}</p>}</div><FeeSummary data={service} compact/><label className="fee-acknowledgement"><input type="checkbox" checked={feeAccepted} onChange={e=>setFeeAccepted(e.target.checked)}/> I agree to the Application Assistance Fee shown above. Official fees are separate.</label><div className="cta-row"><button type="button" className="btn btn-secondary" onClick={()=>setStep(1)}>Edit</button><button type="button" disabled={busy||!feeAccepted} onClick={submit}>{busy?'Submitting…':'Submit request'}</button></div></section>}
  {step===3&&<section className="dashboard-section simplified-confirmation"><span className="eyebrow">Submitted</span><h2>Request received</h2><p className="success-message">{message}</p><div className="request-confirmation"><strong>Request ID</strong><div>{lastOrder?.order_code}</div><p>Status: <strong>{lastOrder?.status||'Submitted'}</strong></p></div><div className="upload-card"><h3>Add a document</h3><p>PDF, PNG or JPG · maximum 10 MB</p><form onSubmit={uploadFile}><input type="file" accept=".pdf,.png,.jpg,.jpeg" onChange={e=>{const selected=e.target.files?.[0]||null;if(selected&&selected.size>10*1024*1024){setUploadMsg('Please choose a file smaller than 10 MB.');return}setFile(selected)}}/><button type="submit" disabled={!file}>Upload</button></form>{uploadMsg&&<p className="info" role="status">{uploadMsg}</p>}</div>{lastOrder?.id&&<RequestFeedback fixedOrderId={lastOrder.id}/>}<div className="cta-row"><button type="button" onClick={()=>navigate('/my-orders')}>Track request</button><Link className="btn btn-secondary" to="/">Done</Link></div></section>}
 </div></>
}
