...
function ServiceInformation({service,requirements}:{service:any,requirements:any}){
 return <section className="service-information" aria-label="Service information">
  <div className="service-info-grid"><article><h3>Details needed</h3><ul>{(requirements.fields||[]).map((field:any)=><li key={field.key}>{field.label}{field.required?' (required)':' (optional)'}</li>)}</ul></article><article><h3>Documents</h3>{requirements.documents?.length?<ul>{requirements.documents.map((item:string)=><li key={item}>{item}</li>)}</ul>:<p>No document needed now.</p>}</article></div>
...
export default function ServiceDetail(){
 ...
 <div className="service-detail-hero"><span className="eyebrow">{service.category||'Public service'}</span><h1>{service.name}</h1><p>{service.description}</p><FeeSummary data={service}/></div>
  ...
  {step===1&&<section className="dashboard-section request-form simplified-request-form">
    ...
    {fields.length>0&&<><h3>Service details</h3><p className="request-short-hint">Complete every field marked required. Optional information can be left blank.</p>{fields.map(field=><label key={field.key}>{field.label}{field.required&&<span className="required" aria-hidden="true"> *</span>}{field.type==='select'?<select required={field.required} aria-required={field.required} value={answers[field.key]||''} onChange={event=>updateAnswer(field.key,event.target.value)}><option value="">Select</option>{(field.options||[]).map(option=><option key={option} value={option}>{option}</option>)}</select>:<input required={field.required} aria-required={field.required} type={field.type||'text'} placeholder={field.placeholder} value={answers[field.key]||''} onChange={event=>updateAnswer(field.key,event.target.value)}/>}</label>)}</>}
    ...
    {requirements.documents?.length>0&&<div className="upload-card"><h3>Documents for this service</h3><ul>{requirements.documents.map((item:string)=><li key={item}>{item}</li>)}</ul><label>Upload documents (multiple)<input type="file" multiple accept=".pdf,.png,.jpg,.jpeg" onChange={event=>{const selected=Array.from(event.target.files||[]);if(selected.length>20){setError('You can upload a maximum of 20 documents for one request.');event.target.value='';setFiles([]);return}const oversized=selected.find(item=>item.size>10*1024*1024);if(oversized){setError(`${oversized.name} is larger than 10 MB.`);event.target.value='';setFiles([]);return}setError('');setFiles(selected)}}/></label><small>Select one or multiple PDF, PNG or JPG files · maximum 20 files · maximum 10 MB each</small>{files.length>0&&<ul className="document-list">{files.map(item=><li key={`${item.name}-${item.lastModified}`}>{item.name}</li>)}</ul>}</div>}
    <p className="request-safety-line">Do not enter OTPs, passwords, PINs or banking details.</p>
    <div className="cta-row"><button type="button" onClick={()=>{if(form.client_name.trim().length<2||form.phone.trim().length<7){setError('Please complete your name and phone number in Account Settings.');return}const missing=fields.filter(field=>field.required&&!String(answers[field.key]||'').trim());if(missing.length){setError(`Please complete: ${missing.map(field=>field.label).join(', ')}.`);return}setRequestMode('guided');setError('');setStep(2)}}>Review request</button></div>
  </section>}
