...
const JOB_FIELDS:Field[]=[
 ...
 {key:'gender',label:'Gender',type:'select',options:['Male','Female','Transgender','Prefer not to say']},
 {key:'category',label:'Category / community',placeholder:'General / EWS / OBC / SC / ST, if applicable'},
 ...
 {key:'reservation_details',label:'Reservation / EWS / PwBD details',placeholder:'Only if applicable'},
...
function ServiceInformation({service,requirements,job}:{service:any,requirements:any,job?:JobNotification|null}){
 return <section className="service-information" aria-label="Service information">
  {job&&<div className="dashboard-section"><span className="eyebrow">Official job selected</span><h3>{job.title}</h3><p>{job.organization}</p><small>Last date: {formatJobDate(job.deadline)} · Official application fee: {job.application_fee||'As stated in the official notice'}</small></div>}
  <div className="service-info-grid"><article><h3>Details you can add</h3><ul>{(requirements.fields||[]).map((field:any)=><li key={field.key}>{field.label} (optional)</li>)}</ul></article><article><h3>Documents</h3>{requirements.documents?.length?<ul>{requirements.documents.map((item:string)=><li key={item}>{item} (optional at request stage)</li>)}</ul>:<p>No document needed now.</p>}</article></div>
...
export default function ServiceDetail(){
 ...
 const [form,setForm]=useState({client_name:'',phone:'',email:'',notes:''});const [answers,setAnswers]=useState<Record<string,string>>({});const [files,setFiles]=useState<File[]>([]);const [message,setMessage]=useState('');const [error,setError]=useState('');const [busy,setBusy]=useState(false);const [uploadMsg,setUploadMsg]=useState('');const [lastOrder,setLastOrder]=useState<any>(null);const [feeAccepted,setFeeAccepted]=useState(false);const [requestMode,setRequestMode]=useState<'guided'|'express'>('guided');const [job,setJob]=useState<JobNotification|null>(null);const [jobLoading,setJobLoading]=useState(false)
 ...
 const requirements=useMemo(()=>job?{
   ...baseRequirements,
   fields:[...(baseRequirements.fields||[]).map((field:any)=>({...field,required:false})),...JOB_FIELDS,...jobSpecificFields(job)].filter((field:any,index:number,all:any[])=>all.findIndex(item=>item.key===field.key)===index),
   documents:[...(baseRequirements.documents||[]),...JOB_DOCUMENTS].filter((item:string,index:number,all:string[])=>all.indexOf(item)===index),
 }:baseRequirements,[service?.id,job?.id])
 ...
 <div className="service-detail-hero"><span className="eyebrow">{service.category||'Public service'}</span><h1>{job?`Apply ${job.title}`:service.name}</h1><p>{job?`${job.organization} · Last date ${formatJobDate(job.deadline)}`:service.description}</p><FeeSummary data={service}/>{job&&<p className="info">Official application fee: <strong>{job.application_fee||'As applicable under the official notice'}</strong>. Assistance fee ₹{Number(service.price_inr||30)} is charged separately.</p>}</div>
  ...
  {step===1&&<section className="dashboard-section request-form simplified-request-form">
    ...
    <div className="cta-row"><button type="button" onClick={()=>{if(form.client_name.trim().length<2||form.phone.trim().length<7){setError('Please complete your name and phone number in Account Settings.');return}setRequestMode('guided');setError('');setStep(2)}}>Review request</button></div>
  </section>}
  {step===2&&<section className="dashboard-section simplified-review"><span className="eyebrow">Step 2</span><h2>Review and submit</h2><div className="request-summary"><p><strong>{job?'Job':'Service'}:</strong> {job?`${job.title} — ${job.organization}`:service.name}</p><p><strong>Client:</strong> {form.client_name} · {form.phone}</p>{requestMode==='guided'&&fields.filter(f=>answers[f.key]).map(f=><p key={f.key}><strong>{f.label}:</strong> {answers[f.key]}</p>)}{form.notes&&<p><strong>Notes:</strong> {form.notes}</p>}</div><FeeSummary data={service} compact/>{job&&<p className="info"><strong>Official application fee:</strong> {job.application_fee||'As applicable under the official notice'}. This is separate from the assistance fee and must be confirmed on the official portal.</p>}<label className="fee-acknowledgement"><input type="checkbox" checked={feeAccepted} onChange={e=>setFeeAccepted(e.target.checked)}/> I agree to the Applicable Assistance Fee shown above. Official fees are separate.</label><div className="cta-row"><button type="button" className="btn btn-secondary" onClick={()=>setStep(1)}>Edit</button><button type="button" disabled={busy||!feeAccepted} onClick={submit}>{busy?'Submitting…':'Submit request'}</button></div></section>}
