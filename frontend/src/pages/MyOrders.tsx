...
export default function MyOrders(){
  ...
  if(error) return <div className="dashboard-state error-state"><h2>Something went wrong</h2><p>{error}</p></div>

  return (
    <div className="client-dashboard">
      <section className="dashboard-hero">
        <div>
          <span className="eyebrow">Citizen workspace</span>
          <h1>My service requests</h1>
          <p>Track applications, see progress updates, and get help whenever you need it.</p>
        </div>
      ...
      <section className="dashboard-section">
        <div className="section-header inline">
          <div><span className="eyebrow">Activity</span><h2>Recent requests</h2></div>
          <Link className="text-link" to="/account-settings">Account settings →</Link>
        </div>
        ...
        {orders.length === 0 ? (
          <div className="empty-dashboard">
            ...
            <p>Choose a public service to start your first request.</p>
            <Link className="btn btn-primary" to="/">Browse services</Link>
          </div>
        ) : (
            ...
            {orders.map(o => (
              <article key={o.id} className="request-card">
                <div className="request-card-main">
                  <div className="request-icon">P</div>
                  <div>
                    <div className="request-code">{o.order_code}</div>
                    <h3>{o.service}</h3>
                    <p>Submitted {new Date(o.created_at).toLocaleString()}</p>
                  </div>
                </div>
                <div className="request-card-side">
                  <span className={`status-pill status-${String(o.status || '').toLowerCase().replace(/\s+/g,'-')}`}>{o.status}</span>
                  <strong>₹{o.fee_inr}</strong>
                </div>
                <div className="request-card-actions">
                  <Link to="/submit-grievance">Need help?</Link>
                  <Link to="/submit-review">Leave a review</Link>
                </div>
              </article>
            ))}
          </div>
