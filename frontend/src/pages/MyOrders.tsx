...
export default function MyOrders(){
  ...
  useEffect(()=>{load()},[])
  const summary=useMemo(()=>({total:orders.length,active:orders.filter(o=>!CLOSED.includes(o.status)).length,completed:orders.filter(o=>o.status==='Completed').length}),[orders])
  ...
  const applications=<section className="dashboard-section" id="applications"><div className="section-header inline"><div><h2>{view==='track'?'Track My Request':'My Applications'}</h2><p>Government services, jobs, scholarships, recharge and bill requests all appear here.</p></div></div>
    <div className="application-toolbar"><label>Search applications<input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Order number, service, job or payment request"/></label><div className="filter-tabs" role="group" aria-label="Filter applications">{FILTERS.map(item=><button type="button" className={filter===item?'active':''} aria-pressed={filter===item} onClick={()=>setFilter(item)} key={item}>{item}</button>)}</div></div>
    {orders.length===0?<div className="empty-dashboard"><h3>No applications yet</h3><p>Start with one of the main services above.</p></div>:filteredOrders.length===0?<div className="empty-dashboard"><h3>No matching applications</h3></div>:<div className="request-list">{filteredOrders.map(order=>{const paid=paymentState[order.id]||{assistance_fee:false,official_fee:false};const anyPaid=paid.assistance_fee||paid.official_fee;const application=order.application_data||{};const requestedAmount=Number(application.requested_payment_amount_inr||application.requested_amount_inr||0);return <article key={order.id} className="request-card"><div className="request-card-main"><div className="request-icon">A</div><div><div className="request-code">{order.order_code}</div><h3>{applicationName(order)}</h3>{applicationName(order)!==order.service&&<p>{order.service}</p>}{requestedAmount>0&&<p><strong>Requested recharge/bill amount: ₹{requestedAmount.toFixed(2)}</strong></p>}<p>{new Date(order.created_at).toLocaleString()}</p></div></div><div className="request-card-side"><span className={`status-pill status-${String(order.status||'').toLowerCase().replace(/\s+/g,'-')}`}>{order.status}</span><span className="request-fee-mini"><strong>{paid.assistance_fee?`Assistance Fee Paid ₹${order.fee_inr}`:`Applicable Assistance Fee ₹${order.fee_inr}`}</strong><small>{paid.official_fee?`Official fee paid: ₹${order.official_fee_inr||0}`:`Official fee: ${order.official_fee_status==='unconfirmed'?'To be confirmed':`₹${order.official_fee_inr||0}`}`}</small></span></div>{order.status==='Cancelled'&&anyPaid&&<div className="info" role="status"><strong>{REFUND_ASSISTANCE_MESSAGE}</strong><br/>Administrator: {PROVIDER.name}<br/>Phone: <a href={`tel:${PROVIDER.phone}`}>{PROVIDER.phone}</a><br/>Email: <a href={`mailto:${PROVIDER.email}`}>{PROVIDER.email}</a></div>}<div className="request-card-actions"><Link to={`/my-orders/${order.id}`}>View application</Link>{!CLOSED.includes(order.status)&&<Link to={`/submit-grievance?order_id=${order.id}`}>Get help</Link>}</div></article>})}</div>}
  </section>
  ...
  return <div className="client-workspace-shell"><ClientWorkspaceNav/><div className="client-dashboard">
    <section className="dashboard-hero"><div><span className="eyebrow">My workspace</span><h1>{view==='applications'?'My Applications':view==='track'?'Track My Request':view==='service-search'?'Find Services':view==='notifications'?'Notifications':`Welcome${profile?.name?`, ${profile.name}`:''}`}</h1><p>{view?'Your secure account workspace.':'Services, opportunities, payments and request tracking in one place.'}</p></div><span className="dashboard-safety">✓ Never share OTPs, passwords, PINs or CVV</span></section>
    {loading&&<div className="dashboard-state" role="status"><div className="loading-dot"/><p>Loading your workspace…</p></div>}
    ...
    {!view&&<><section className="workspace-launch-grid" aria-label="Main client actions"><Link className="workspace-launch-card" to="/government-services"><i>▤</i><span><strong>Government Services</strong><small>Certificates, identity, land, welfare and other services</small></span></Link><Link className="workspace-launch-card" to="/jobs"><i>▣</i><span><strong>Government & Private Jobs</strong><small>Official-source opportunities checked and updated daily</small></span></Link><Link className="workspace-launch-card" to="/scholarships"><i>◆</i><span><strong>Scholarships</strong><small>Current scholarship opportunities checked daily</small></span></Link><Link className="workspace-launch-card" to="/recharge-bills"><i>₹</i><span><strong>Recharge & Pay Bills</strong><small>Choose a biller, enter details, review and track the request</small></span></Link></section><section className="dashboard-stat-grid"><div className="dashboard-stat"><span>Applications</span><strong>{summary.total}</strong></div><div className="dashboard-stat"><span>Active</span><strong>{summary.active}</strong></div><div className="dashboard-stat"><span>Completed</span><strong>{summary.completed}</strong></div><div className="dashboard-stat"><span>Unread</span><strong>{unread}</strong></div></section><JobRecommendations profile={profile}/><section className="dashboard-section"><div className="dashboard-section-heading"><div><h2>Quick actions</h2><p>Continue any common task without returning to the homepage.</p></div></div><div className="dashboard-action-grid"><Link className="action-card" to="/my-orders#service-search"><strong>Search all services</strong><small>Find the right service quickly</small></Link><Link className="action-card" to="/my-orders#applications"><strong>My Applications</strong><small>Review status and next steps</small></Link><Link className="action-card" to="/my-orders#notifications"><strong>Notifications</strong><small>See updates that need attention</small></Link><Link className="action-card" to="/grievances"><strong>Help & Grievances</strong><small>Contact the service administrator</small></Link></div></section><section className="dashboard-help-strip"><div><strong>Need a recharge or bill payment?</strong><p>Use the dedicated guided flow so the account number and amount are checked before submission.</p></div><Link className="btn btn-primary" to="/recharge-bills">Open Recharge & Bills</Link></section>{orders.length>0&&applications}{orders.length>0&&<section className="dashboard-section" id="feedback"><RequestFeedback orders={orders}/></section>}</>}
    </>}
  </div></div>
}
