...
export default function MyOrders(){
  ...
  return <div className="client-dashboard">
    ...
    {notifications.length>0&&<section className="dashboard-section"><div className="section-header inline"><div><span className="eyebrow">Updates</span><h2>Recent notifications</h2></div>{unread>0&&<button onClick={async()=>{await axios.post(`${apiBase}/notifications/read-all`,{}, {headers:authHeader()});setUnread(0);setNotifications(items=>items.map(item=>({...item,is_read:true})))}}>Mark all read</button>}</div><div className="notification-list">{notifications.slice(0,5).map(item=><article key={item.id} className={item.is_read?'notification-card':'notification-card unread'}><strong>{item.title}</strong><p>{item.message}</p><small>{new Date(item.created_at).toLocaleString()}</small>{item.order_id&&<Link to={`/my-orders/${item.order_id}`}>View application</Link>}</article>)}</div></section>}
