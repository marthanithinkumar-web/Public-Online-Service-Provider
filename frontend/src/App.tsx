...
export default function App(){
  ...
  return <div className="app-shell">
    <header className={`site-header ${isAdmin?'admin-header':''}`}>
      ...
      <div className="container provider-strip"><span>{PROVIDER.name}</span><span><a href={`tel:${PROVIDER.phone}`}>{PROVIDER.phone}</a> · <a href={`mailto:${PROVIDER.email}`}>{PROVIDER.email}</a></span></div>
    </header>
