...
export default function OrderManagement(){
 ...
 return <div className="admin-orders"><div className="section-header"><div><span className="eyebrow">Request queue</span><h2>{status||'All requests'}</h2><p>Open a request to process its application, documents, timeline and status.</p></div></div>
  ...
  {loading?<div className="dashboard-state"><div className="loading-dot"/><p>Loading requests...</p></div>:!orders.length&&!error?<div className="dashboard-state"><p>No requests found for this filter.</p></div>:<ul className="admin-record-list">{orders.map(o=><li key={o.id}><div className="admin-record-main"><span className="request-code">{o.order_code}</span><strong>{o.service}</strong><small>{o.client_name} · {o.phone}</small></div><span className={`status-pill status-${String(o.status||'').toLowerCase().replace(/\s+/g,'-')}`}>{o.status}</span><div className="admin-record-fees"><small>Assistance ₹{o.fee_inr}</small><small>Official: {o.official_fee_status==='unconfirmed'?'To be confirmed':`₹${o.official_fee_inr||0}`}</small></div><Link className="btn btn-secondary" to={`/admin/orders/${o.id}`}>Open application</Link></li>)}</ul>}
