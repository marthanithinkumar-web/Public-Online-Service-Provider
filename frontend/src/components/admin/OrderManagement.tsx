...
export default function OrderManagement(){
 ...
 const [orders,setOrders]=useState<any[]>([]);const [meta,setMeta]=useState<any>({});const [page,setPage]=useState(1);const [loading,setLoading]=useState(true);const [error,setError]=useState('')
 const load=async(p=1)=>{setLoading(true);setError('');try{const res=await fetchAdminOrders(p,20,status);setOrders(res.items||[]);setMeta(res.meta||{})}catch(err:any){setError(err?.response?.data?.error||'Unable to load requests. Please try again.')}finally{setLoading(false)}}
 ...
 return <div className="admin-orders"><div className="section-header"><div><span className="eyebrow">Request queue</span><h2>{status||'All requests'}</h2><p>Open a request to process its application, documents, timeline and status.</p></div></div>
  <label>Status filter<select value={status} onChange={e=>{const next=e.target.value;if(next)params.set('status',next);else params.delete('status');setParams(params)}}>{STATUSES.map(s=><option key={s} value={s}>{s||'All statuses'}</option>)}</select></label>
