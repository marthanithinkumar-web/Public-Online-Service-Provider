...
export default function OrderManagement(){
 ...
 return <div className="admin-orders"><div className="section-header"><div><span className="eyebrow">Request queue</span><h2>{status||'All requests'}</h2><p>Open a request to process its application, documents, timeline and status.</p></div></div>
  ...
  {loading?<div className="dashboard-state"><div className="loading-dot"/><p>Loading requests...</p></div>:!orders.length&&!error?<div className="dashboard-state"><p>No requests found for this filter.</p></div>:<ul>{orders.map(o=><li key={o.id} style={{border:'1px solid #eee',padding:12,margin:'8px 0',borderRadius:8}}><div><strong>{o.order_code}</strong> — {o.service}</div><div>{o.client_name} • {o.phone}</div><div>Status: {o.status}</div><div>Assistance fee: ₹{o.fee_inr}</div><div>Official fee: {o.official_fee_status==='unconfirmed'?'To be confirmed':`₹${o.official_fee_inr||0}`}</div><Link className="btn btn-secondary" to={`/admin/orders/${o.id}`}>Open request</Link></li>)}</ul>}
