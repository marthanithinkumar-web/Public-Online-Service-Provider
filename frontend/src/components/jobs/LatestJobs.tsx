...
export default function LatestJobs(){
  ...
  useEffect(()=>{let active=true;fetchJobs({limit:9}).then(data=>{if(active)setJobs(data.items||[])}).catch(()=>{if(active)setError('Latest job notices are temporarily unavailable. Service search remains available below.')} ).finally(()=>{if(active)setLoading(false)});return()=>{active=false}},[])
  const visible=useMemo(()=>jobs.filter(job=>filter==='all'||job.job_type===filter).slice(0,3),[jobs,filter])
