...
export default function LatestJobs({maxItems=4}:{maxItems?:number}){
  ...
  const visible=useMemo(()=>jobs.filter(job=>filter==='all'||job.job_type===filter).slice(0,maxItems),[jobs,filter,maxItems])
