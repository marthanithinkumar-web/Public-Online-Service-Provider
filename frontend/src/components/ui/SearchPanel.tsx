...
export default function SearchPanel({context='home'}:{context?:'home'|'dashboard'}){
  ...
  return <section className="content-section search-panel-premium" id="service-search" aria-labelledby={titleId}>
    <div className="section-header inline"><div><span className="eyebrow">{context==='dashboard'?'Explore services':'Quick search'}</span><h2 id={titleId}>What service do you need help with?</h2></div><span className="search-meta" aria-live="polite">{loading?'Loading services':term?`${visible.length} result${visible.length===1?'':'s'}`:'Ready'}</span></div>
    <div className="search-box"><input ref={inputRef} id={inputId} type="search" autoComplete="off" aria-label="Search services" placeholder="Search jobs, scholarships, certificates, schemes..." value={q} onChange={e=>{setQ(e.target.value);setCategory('')}}/>{q&&<button type="button" className="btn-secondary" onClick={()=>{setQ('');setCategory('');inputRef.current?.focus()}} aria-label="Clear service search">Clear</button>}</div>
