...
export default function SearchPanel({context='home',variant='section'}:{context?:'home'|'dashboard',variant?:'section'|'hero'}){
  ...
  return <section className={variant==='hero'?'hero-search-panel':'content-section search-panel-premium'} id="service-search" aria-labelledby={titleId}>
    ...
    {!loading&&!error&&visible.length>0&&<ul className="service-list" aria-live="polite">{visible.map(service=><li key={service.id} className="service-card"><div className="service-card-top"><span className="service-badge">{categoryName(service)}</span><span className="service-price">Application Assistance Fee ₹{service.price_inr}</span></div><h3><Link to={servicePath(service)}>{service.name}</Link></h3><Link className="text-link" to={servicePath(service)}>View & apply</Link></li>)}</ul>}
  </section>
}
