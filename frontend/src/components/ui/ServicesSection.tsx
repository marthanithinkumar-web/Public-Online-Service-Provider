...
export default function ServicesSection(){
  ...
  return <section className="content-section services-section" aria-labelledby="popular-services-title">
    ...
    {loading&&popular.length===0?<div className="empty-state" role="status"><div className="loading-dot"/><p>Loading…</p></div>:<div className="popular-service-grid">{popular.map(service=><article key={service.id} className="popular-service-card"><div className="popular-service-heading"><span className="popular-service-icon" aria-hidden="true">{String(service.name||'S').charAt(0)}</span><div><span className="service-badge">{categoryName(service)}</span><h3>{service.name}</h3></div></div><div className="popular-service-meta"><span><small>Applicable Assistance Fee</small><strong>{service.price_inr==null?'To be confirmed':`₹${Number(service.price_inr)}`}</strong></span><span><small>Official fee</small><strong>{service.official_fee_status==='known'?`₹${Number(service.official_fee_inr??0)}`:service.official_fee_status==='none'?'₹0':'To be confirmed'}</strong></span></div><Link className="btn btn-secondary small" to={servicePath(service)}>Apply</Link></article>)}</div>}
  </section>
}
