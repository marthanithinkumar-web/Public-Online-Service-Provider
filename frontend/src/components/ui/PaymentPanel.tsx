...
export default function PaymentPanel({order}:{order:any}){
  ...
  const total=state?.breakdown?.combined_total_inr==null?null:Number(state.breakdown.combined_total_inr)
  const assistancePaid=Boolean(state?.paid_components?.assistance_fee)
  const officialPaid=Boolean(state?.paid_components?.official_fee)||officialStatus==='none'||official===0
  ...
  return <section className="dashboard-section payment-panel" aria-labelledby="request-payment-title">
    ...
    <p>Choose how you want to pay for this request. You can pay the assistance fee first, the official fee separately after it is confirmed, or pay both together when both are still due.</p>
    <div className="payment-options-grid">
      {card('assistance_fee','Pay Assistance Fee','Pay only our assistance fee now.',assistance,assistancePaid,assistancePaid)}
      {card('official_fee','Pay Official Fee','Pay only the confirmed official/government fee.',official,officialStatus==='unconfirmed'||officialPaid,officialPaid)}
      {card('request_total','Pay Total (Both Fees)','Pay the assistance fee and official fee together in one transaction.',total,officialStatus==='unconfirmed'||assistancePaid||officialPaid,assistancePaid&&officialPaid,!assistancePaid&&!officialPaid&&officialStatus!=='unconfirmed')}
    </div>
    <div className="payment-summary-box"><h3>Payment Summary</h3><div><span>Assistance fee</span><strong>₹{assistance.toFixed(2)}</strong></div><div><span>Official / Government fee</span><strong>{officialStatus==='unconfirmed'?'To be confirmed':`₹${Number(official||0).toFixed(2)}`}</strong></div><div className="payment-summary-total"><span>Total</span><strong>{total==null?'Confirmed later':`₹${total.toFixed(2)}`}</strong></div><small>You may pay fees individually or together for your convenience.</small></div>
    {officialStatus==='unconfirmed'&&!officialPaid&&<p className="info">The official fee is still being confirmed. You can pay the assistance fee now. Once the admin confirms the official fee, you can pay that fee separately.</p>}
