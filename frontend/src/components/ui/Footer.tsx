  ...
  return (
    <footer className="site-footer">
      <div className="container footer-grid">
        <div>
          <Link to="/" className="brand-lockup footer-lockup"><img src="/assets/logo.svg" alt="logo" className="brand-mark"/><span><strong className="brand footer-brand">Public Online Service Provider</strong><small>Simple. Secure. Citizen-focused.</small></span></Link>
        ...
        <div>
          <h4>Contact</h4>
          <div className="footer-links"><a href={`tel:${PROVIDER.phone}`}>{PROVIDER.phone}</a> · <a href={`tel:${PROVIDER.phone2}`}>{PROVIDER.phone2}</a> · <a href={`mailto:${PROVIDER.email}`}>{PROVIDER.email}</a><span>{PROVIDER.name}</span></div>
        </div>
      </div>
